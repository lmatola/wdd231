...
const myHighTemp = document.querySelector("#high-temp");
const myLowTemp = document.querySelector("#low-temp");
...
function displayCurrentWeather(data) {
    ...
    myHighTemp.innerHTML = `High: ${highTemp} &deg;C`;
    myLowTemp.innerHTML = `Low: ${lowTemp} &deg;C`;
    myHumidity.innerHTML = `humidity: ${humidity}%`;
    mySunRise.innerHTML = `Sunrise: ${sunrise}`;
    mySunSet.innerHTML = `Sunset: ${sunset}`;
...
function displayForecast(data) {
    ...
    for (let i = 0; i < data.list.length; i += 8) {
        ...
        forecastItem.textContent = `${dayLabel}: ${temp} °C`;
